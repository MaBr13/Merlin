...
Departures_Autumn_int$country <- factor(Departures_Autumn_int$country, levels = c("Norway", "Sweden","Denmark","Germany","The Netherlands", "Belgium",
                                                                    "UK & Ireland","France","Other"),
                                 labels  = c("Norway", "Sweden","Denmark","Germany","The Netherlands", "Belgium",
                                             "UK & Ireland","France","Other"))
Departures_Spring_int$country <- factor(Departures_Spring_int$country, levels = c("Norway", "Sweden","Denmark","Germany","The Netherlands", "Belgium",
                                                                    "UK & Ireland","France","Other"),
                                 labels  = c("Norway", "Sweden","Denmark","Germany","The Netherlands", "Belgium",
                                             "UK & Ireland","France","Other"))
library(ggplot2)
...
windows(7,4)
p1 <- ggplot(subset(Departures_int,month(date)>=2 & month(date)<=5),aes(x = factor(date), fill = country)) + 
  ...
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 30, hjust = 1),plot.margin = unit(c(.5,1,.5,1), "cm"))
windows(8,9) 
fplot <- ggarrange(p1,p2,labels = c("(a)","(b)"),font.label = list(size = 12),ncol=1,nrow = 2,
                  common.legend = TRUE,legend ="bottom",label.x=c(0.93,0.93),label.y = c(1,1))+
  theme(plot.margin = unit(c(0.1, 0.1, 0.1, 0.1), "cm"), legend.text = element_text(size = 4))
ggsave(filename=paste0("C:/Users/mbradar/surfdrive/Documents/Maja_PhD/first_paper_ver1/Paper figures/individual_days.png"),fplot,dpi=500)
...
s1 <- ggplot(subset(Departures_int,month(date)>=8 & month(date)<=11),aes(x = airspeed, fill = country)) + 
  ...
  scale_colour_manual(values = rep("black",7))+
  scale_fill_brewer(palette="PuBuGn",name="")+
  theme(legend.position=c(0.85, 0.75),legend.text = element_text(size=12),
