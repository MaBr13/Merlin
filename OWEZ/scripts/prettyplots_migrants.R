#plot a nice grid of nocturnal and diurnal migrants throughout the years
require(ggplot2)
...
for (k in 1:length(AutumnAll)){
  ...
  p1 <-  ggplot(means[[1]],aes(Rlight, Nr.tracks))+
    ...
    scale_fill_manual(values = c("grey","grey0","forestgreen","coral3"), name="Air speed (kph)", drop=F)+
    scale_colour_manual(values  = c("grey","grey0","forestgreen","coral3"), name="Air speed (kph)", drop=F)+
    ylim(0,1250000)+
    ggtitle("2007") +
...
######wind rose (wind direction) in Spring
SpringAll <- list()
for(k in 1:length(Allyears)){
  
  SpringAll[[k]] <- subset(Allyears[[k]], season==2, select=id:DirectionS)
}
      ...
      scale_colour_manual(values = c("black", "black", "black",  "black"), name="Wind speed (kph)", drop=F)+
    ...
    spwS3 <- ggplot(SpringAll[[3]], aes(x=winddir)) + 
      ...
      scale_colour_manual(values = c("black", "black", "black",  "black"), name="Wind speed (kph)", drop=F)+
      scale_fill_manual(values = c("grey", "yellow","navy","steelblue"), name="Wind speed (kph)", drop=F)+
      ggtitle("Wind rose") +
...
####windrose (wind direction) Autumn
AutumnAll <- list()
    for(k in 1:length(Allyears)){
      
      AutumnAll[[k]] <- subset(Allyears[[k]], season==4, select=id:DirectionS)
    } 
...
spwA2 <- ggplot(AutumnAll[[2]], aes(x=winddir)) + 
...
sphS1 <- ggplot(SpringAll[[1]], aes(x=trackheading)) + 
  ...
  scale_colour_manual(values = c("black", "black", "black",  "black"), name="Air speed (kph)", drop=F)+
...
sphS3 <- ggplot(SpringAll[[3]], aes(x=trackheading)) + 
...
sphA2 <- ggplot(AutumnAll[[2]], aes(x=trackheading)) + 
...
for(k in 1:length(SpringAll)){
  ...
  NoctS[[k]] <- subset(SpringAll[[k]], light==0, select=id:DirectionS)
}

for (k in 1:length(NoctS)){
  NoctS[[k]]$Aspeed <- factor(NoctS[[k]]$Aspeed, levels = rev(levels(NoctS[[k]]$Aspeed)))
  
}
#Wspeed

for (k in 1:length(NoctS)){
  NoctS[[k]]$Wspeed <- factor(NoctS[[k]]$Wspeed, levels = rev(levels(NoctS[[k]]$Wspeed)))
  
}

snhS1 <- ggplot(NoctS[[1]], aes(x=trackheading)) + 
  geom_histogram(aes(fill=NoctS[[1]]$Aspeed, colour=NoctS[[1]]$Aspeed), breaks=c(0,30,60,90,120,150,180,210,240,270,300,330,360)) +
  scale_colour_manual(values = c("black", "black", "black",  "black"), name="Air speed (kph)", drop=F)+
  scale_fill_manual(values = c("coral3", "forestgreen", "grey0", "grey"), name="Air speed (kph)", drop=F)+
  ggtitle(2007) + 
  ylab("Number of tracks")+ ylim(0,70000)+
  theme(axis.title.y = element_text(size=14), axis.title.x = element_blank(), plot.title = element_text(size = 16))+
  coord_polar(start = 0) +
  scale_x_continuous("",limits=c(0,360), breaks = c(0,30,60,90,120,150,180,210,240,270,300,330,360))

snhS2 <- ggplot(NoctS[[2]], aes(x=trackheading)) + 
  geom_histogram(aes(fill=NoctS[[2]]$Aspeed, colour=NoctS[[2]]$Aspeed), breaks=c(0,30,60,90,120,150,180,210,240,270,300,330,360)) +
  scale_colour_manual(values = c("black", "black", "black",  "black"), name="Air speed (kph)", drop=F)+
  scale_fill_manual(values = c("grey","grey0","forestgreen","coral3"), name="Air speed (kph)", drop=F)+
  ggtitle(2008) + 
  ylab("Number of tracks")+ ylim(0,70000)+
...
snhS3 <- ggplot(NoctS[[3]], aes(x=trackheading)) + 
  geom_histogram(aes(fill=NoctS[[3]]$Aspeed, colour=NoctS[[3]]$Aspeed), breaks=c(0,30,60,90,120,150,180,210,240,270,300,330,360)) +
  scale_colour_manual(values = c("black", "black", "black",  "black"), name="Air speed (kph)", drop=F)+
  scale_fill_manual(values = c("grey","grey0","forestgreen","coral3"), name="Air speed (kph)", drop=F)+
  ggtitle("Flight direction") + 
  ylab("Number of tracks")+ ylim(0,60000)+
  theme(axis.title.y = element_text(size=18), legend.text=element_text(size=12), 
        legend.title=element_text(size=16, face="bold"), legend.position = "bottom",
        axis.text.y=element_text(size=14), axis.text.x=element_text(size=14), plot.margin = unit(c(0,0,0,0), "cm"),
        axis.title.x = element_blank(), plot.title = element_text(size = 18, face = "bold"))+
...
snhS4 <- ggplot(NoctS[[4]], aes(x=trackheading)) + 
  geom_histogram(aes(fill=NoctS[[4]]$Aspeed, colour=NoctS[[4]]$Aspeed), breaks=c(0,30,60,90,120,150,180,210,240,270,300,330,360)) +
  scale_colour_manual(values = c("black", "black", "black",  "black"), name="Air speed (kph)", drop=F)+
  scale_fill_manual(values = c("coral3", "forestgreen", "grey0", "grey"), name="Air speed (kph)", drop=F)+
  ggtitle(2010) + 
  ylab("Number of tracks")+ ylim(0,70000)+
...
library(gridExtra)
library(grid)
snheadS <- ggarrange(snhS1, snhS2,snhS3, snhS4,  ncol=4, nrow=1, common.legend = TRUE, legend="bottom",widths=c(1.31,1,1,1)) +
  theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))
annotate_figure(snheadS,top = textGrob("Headings and air speeds Spring (Nocturnal)",gp=gpar(fontsize=20,fontface="bold")))   

snwS3 <- ggplot(NoctS[[3]], aes(x=winddir)) + 
  geom_histogram(aes(fill=NoctS[[3]]$Wspeed, colour=NoctS[[3]]$Wspeed), breaks=c(0,30,60,90,120,150,180,210,240,270,300,330,360)) +
  scale_colour_manual(values = c("black", "black", "black",  "black"), name="Wind speed (kph)", drop=F)+
  scale_fill_manual(values = c("grey", "yellow","navy","steelblue"), name="Wind speed (kph)", drop=F)+
  ggtitle("Wind rose") + 
  ylab("Number of tracks")+ ylim(0,60000)+
...
for(k in 1:length(AutumnAll)){
  ...
  NoctA[[k]] <- subset(AutumnAll[[k]], light==0, select=id:DirectionS)
}

for (k in 1:length(NoctA)){
  NoctA[[k]]$Aspeed <- factor(NoctA[[k]]$Aspeed, levels = rev(levels(NoctA[[k]]$Aspeed)))
  
}
#Wspeed

for (k in 1:length(NoctA)){
  NoctA[[k]]$Wspeed <- factor(NoctA[[k]]$Wspeed, levels = rev(levels(NoctA[[k]]$Wspeed)))
  
}

snhA1 <- ggplot(NoctA[[1]], aes(x=trackheading)) + 
  geom_histogram(aes(fill=NoctA[[1]]$Aspeed, colour=NoctA[[1]]$Aspeed), breaks=c(0,30,60,90,120,150,180,210,240,270,300,330,360)) +
  scale_colour_manual(values = c("black", "black", "black",  "black"), name="Air speed (kph)", drop=F)+
  scale_fill_manual(values = c("coral3", "forestgreen", "grey0", "grey"), name="Air speed (kph)", drop=F)+
  ggtitle(2007) + 
  ylab("Number of tracks")+ ylim(0,100000)+
  theme(axis.title.y = element_text(size=14), axis.title.x = element_blank(), plot.title = element_text(size = 16))+
  coord_polar(start = 0) +
  scale_x_continuous("",limits=c(0,360), breaks = c(0,30,60,90,120,150,180,210,240,270,300,330,360))

snhA2 <- ggplot(NoctA[[2]], aes(x=trackheading)) + 
  geom_histogram(aes(fill=NoctA[[2]]$Aspeed, colour=NoctA[[2]]$Aspeed), breaks=c(0,30,60,90,120,150,180,210,240,270,300,330,360)) +
  scale_colour_manual(values = c("black", "black", "black",  "black"), name="Air speed (kph)", drop=F)+
  scale_fill_manual(values = c("grey","grey0","forestgreen","coral3"), name="Air speed (kph)", drop=F)+
  ggtitle("Flight direction") + 
  ylab("Number of tracks")+ ylim(0,100000)+
...
library(gridExtra)
library(grid)
snheadA <- ggarrange(snhA1, snhA2,  ncol=2, nrow=1, common.legend = TRUE, legend="bottom",widths=c(1.32,1)) +
  theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))
annotate_figure(snheadA,top = textGrob("Headings and air speeds Autumn (Nocturnal)",gp=gpar(fontsize=20,fontface="bold")))

snwA2 <- ggplot(NoctA[[2]], aes(x=winddir)) + 
  geom_histogram(aes(fill=NoctA[[2]]$Wspeed, colour=NoctA[[2]]$Wspeed), breaks=c(0,30,60,90,120,150,180,210,240,270,300,330,360)) +
  scale_colour_manual(values = c("black", "black", "black",  "black"), name="Wind speed (kph)", drop=F)+
  scale_fill_manual(values = c("grey", "yellow","navy","steelblue"), name="Wind speed (kph)", drop=F)+
  ggtitle("Wind rose") + 
  ylab("Number of tracks")+ ylim(0,100000)+
