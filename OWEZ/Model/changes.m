...
%% Title time
    Tdum=DayS-1+datenum([num2str(year),'-01-01 00:00:00']);
    TT=datestr(Tdum(1,1), 'yyyy mm dd'); TT(5)=[]; TT(7)=[];
...
LatS(1:nTracks,1,1)=52.60636;
LongS(1:nTracks,1,1)=4.389639;
DumTime=interpn(SSLat,SSLong,SSTime,LatS(1:nTracks,1,1),LongS(1:nTracks,1,1),'linear', -999);
sunsetT=DumTime+datenum([str2double(TT(1:4)), str2double(TT(5:6)), str2double(TT(7:8))]);
nSteps=2*hms(datetime(Tdum-sunsetT,'ConvertFrom','datenum'));
%% Visualisation
...
for ii=1:nDays
 ...
 for i=1:nSteps      %24 steps = 0.5 day ; 60 steps max
        ...
        if i==1
            ...
            Time(1:nTracks,i,ii)=Time(1:nTracks,i-1,ii)-dt/nSteps;
        end
